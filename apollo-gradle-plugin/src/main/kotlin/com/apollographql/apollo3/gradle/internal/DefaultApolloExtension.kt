...
abstract class DefaultApolloExtension(
    ...
    project.afterEvaluate {
      if (registerDefaultService) {
        ...
        check(defaultService.graphqlSourceDirectorySet.isEmpty
            ...
            && defaultService.extraSchemaFiles.isEmpty
  ...
  private fun registerCodeGenTask(
    ...
    return project.tasks.register(ModelNames.generateApolloSources(service), ApolloGenerateSourcesTask::class.java) { task ->
      ...
      task.extraSchemaFiles.from(service.resolvedExtraSchemaFiles(project))
  ...
  override fun createAllAndroidVariantServices(sourceFolder: String, nameSuffix: String, action: Action<Service>) {
    /**
  ...
  override fun createAllKotlinJvmSourceSetServices(sourceFolder: String, nameSuffix: String, action: Action<Service>) {
    registerDefaultService = false
