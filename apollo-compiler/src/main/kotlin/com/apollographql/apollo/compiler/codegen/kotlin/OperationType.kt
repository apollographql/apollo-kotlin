...
fun KotlinCodeGen.operationTypeSpec(operationType: AST.OperationType): TypeSpec {
...
private val AST.OperationType.superInterfaceType: TypeName
  get() {
    return when (type) {
    ...
    }.parameterizedBy(data.asTypeName(), data.asTypeName(), Operation.Variables::class.asClassName())
  }
...
private val AST.InputType.variablesValueMapSpec: FunSpec
  get() {
    return FunSpec.builder("valueMap")
      ...
      .beginControlFlow("return mutableMapOf<%T, %T>().apply", String::class, Any::class.asClassName().asNullable())
      .addCode(
        fields.map { field ->
          if (field.isOptional) {
