...
object KotlinCodeGen {
  ...
  fun AST.FieldType.asTypeName(): TypeName = when (this) {
    ...
    is AST.FieldType.Array -> List::class.asClassName().parameterizedBy(rawType.asTypeName())
  }
  ...
  private fun AST.FieldType.readCode(reader: String, field: String): CodeBlock {
    return when (this) {
      ...
      is AST.FieldType.Object -> {
        ...
        CodeBlock.builder()
            .beginControlFlow("%L.readObject<%T>%L", reader, typeRef.asTypeName(), fieldCode)
            .addStatement("%T(it)", typeRef.asTypeName())
            .endControlFlow()
            .build()
      }
