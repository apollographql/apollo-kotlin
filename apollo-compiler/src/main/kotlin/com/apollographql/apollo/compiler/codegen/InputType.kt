      ...
      is CodeGenerationAst.FieldType.Scalar.Custom -> {
        if (type.nullable) {
          CodeBlock.builder()
              .addStatement("if·(this@%L.%L.defined)·{", thisRef, name)
              .indent()
              .addStatement("writer.writeCustom(%S, %T, this@%L.%L.value)", schemaName, type.customEnumType.asTypeName(), thisRef, name)
              .unindent()
              .addStatement("}")
              .build()
        } else {
          CodeBlock.of("writer.writeCustom(%S, %T, this@%L.%L)\n", schemaName, type.customEnumType.asTypeName(), thisRef, name)
        }
      }
    }
