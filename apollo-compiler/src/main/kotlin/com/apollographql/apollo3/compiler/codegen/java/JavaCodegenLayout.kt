...
class JavaCodegenLayout(
  ...
  // ------------------------ Helpers ---------------------------------
  private fun regularIdentifier(name: String) = name.escapeJavaReservedWord()
  private fun capitalizedIdentifier(name: String): String {
    return name.capitalizeFirstLetter().escapeJavaReservedWord()
  }

  fun rootSelectionsPropertyName() = "root"

  companion object {
    private fun upperCamelCaseIgnoringNonLetters(strings: Collection<String>): String {
      return strings.map {
