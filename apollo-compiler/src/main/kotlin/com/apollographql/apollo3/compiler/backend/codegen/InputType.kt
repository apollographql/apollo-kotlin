  ...
  return when (type) {
    is CodeGenerationAst.FieldType.Scalar -> when (type) {
      ...
      is CodeGenerationAst.FieldType.Scalar.String -> {
        if (type.nullable) {
          CodeBlock.builder()
              .addStatement("if·(this@%L.%L·is·%T)·{", thisRef, name.escapeKotlinReservedWord(), Input.Present::class)
              .indent()
      ...
      is CodeGenerationAst.FieldType.Scalar.Custom -> {
        if (type.nullable) {
          CodeBlock.builder()
              .addStatement("if·(this@%L.%L·is·%T)·{", thisRef, name, Input.Present::class)
              .indent()
              .addStatement(
    ...
    is CodeGenerationAst.FieldType.Array -> {
      ...
      if (type.nullable) {
        codeBlockBuilder
            .beginControlFlow("if·(this@%L.%L·is·%T)", thisRef, name, Input.Present::class)
