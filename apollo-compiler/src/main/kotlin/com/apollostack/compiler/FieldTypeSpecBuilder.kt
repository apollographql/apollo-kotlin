...
class FieldTypeSpecBuilder {
  fun build(typeName: String, fields: List<Field>, fragments: List<Fragment>, inlineFragments: List<InlineFragment>?): TypeSpec =
  ...
  private fun innerTypes(fields: List<Field>, fragments: List<Fragment>): List<TypeSpec> =
      fields.filter(Field::isNonScalar)
          .map { build(it.normalizedName(), it.fields!!, it.referencedFragments(fragments), it.inlineFragments) }
