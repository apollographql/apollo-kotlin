...
interface ResponseWriter {
  ...
  fun <T> writeList(field: ResponseField, values: List<T>?, block: (items: List<T>?, listItemWriter: ListItemWriter) -> Unit) {
    writeList(field, values, ListWriter(block))
  }
  ...
  interface ListWriter<T> {
    ...
    companion object {
      @JvmSynthetic
      inline operator fun <T> invoke(crossinline block: (items: List<T>?, listItemWriter: ListItemWriter) -> Unit) =
          object : ListWriter<T> {
  ...
  interface ListItemWriter {
    ...
    @JvmSynthetic
    fun <T> writeList(values: List<T>?, block: (items: List<T>?, listItemWriter: ListItemWriter) -> Unit) {
      writeList(values, ListWriter(block))
    }
  }
}
