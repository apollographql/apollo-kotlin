...
class BuilderProperty<T>(val adapter: ApolloAdapter<T>) {
  operator fun getValue(thisRef: ObjectBuilder<*>, property: kotlin.reflect.KProperty<*>): T {
    ...
    return adapter.deserializeData(MapJsonReader(data), ApolloAdapter.DataDeserializeContext(scalarAdapters = ScalarAdapters.Empty, falseBooleanVariables = emptySet(), mergedDeferredFragmentIds = null))
  }
