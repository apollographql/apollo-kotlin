...
class MultiModulesTests {
  ...
  fun `duplicate fragments are detected correctly`() {
    TestUtils.withTestProject("multi-modules-duplicates") { dir ->
      try {
        ...
        Truth.assertThat(e.message).contains("in modules: :node1:impl,:node2:impl")
      }
    }
  }
  ...
  fun `changing a module does not recompile siblings`() {
    TestUtils.withTestProject("multi-modules-duplicates") { dir ->
      ...
      Truth.assertThat(result.task(":node2:impl:generateServiceApolloSources")?.outcome).isEqualTo(TaskOutcome.UP_TO_DATE)
    }
  }
