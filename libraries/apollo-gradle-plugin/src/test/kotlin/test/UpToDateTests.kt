...
class UpToDateTests {
  ...
  fun `change graphql file rebuilds the sources`() {
    withSimpleProject { dir ->
      ...
      assertEquals(TaskOutcome.SUCCESS, result.task(":generateApolloSources")!!.outcome)
      Truth.assertThat(dir.generatedSource("com/example/DroidDetailsQuery.kt").readText()).contains("classification")
      ...
      assertEquals(TaskOutcome.SUCCESS, result.task(":generateApolloSources")!!.outcome)
      Truth.assertThat(dir.generatedSource("com/example/DroidDetailsQuery.kt").readText()).doesNotContain("classification")
    }
  }
