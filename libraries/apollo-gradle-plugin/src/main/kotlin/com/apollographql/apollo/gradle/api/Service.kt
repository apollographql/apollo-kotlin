...
interface Service {
   ...
   * ```
   *
   * an operation defined in `src/main/graphql/query/feature1` is generated in package `com.example.query.feature1`.
   * an input object defined in `src/main/graphql/schema/schema.graphqls` is generated in package `com.example.schema.type`.
   */
   ...
   */
  @Deprecated("Changing the location of the generated sources is not possible anymore. Use `outputDirConnection` to consume them and potentially copy them to another filesystem location.", level = DeprecationLevel.ERROR)
  val outputDir: DirectoryProperty

  /**
   * The format to output for the operation manifest. Valid values are:
   *
   * - "persistedQueryManifest": the manifest format for [persisted queries](https://www.apollographql.com/docs/kotlin/advanced/persisted-queries)
   * - "none" (Default): no manifest is generated
   *
   * Example of a "persistedQueryManifest":
   * ```json
   * {
   ...
   */
  @Deprecated("Changing the location of the generated manifest is not possible anymore. Use `operationManifestConnection` to consume it and potentially copy it to another filesystem location.", level = DeprecationLevel.ERROR)
