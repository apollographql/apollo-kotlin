...
import com.apollographql.apollo.compiler.codegen.Identifier.__Known
import com.apollographql.apollo.compiler.codegen.Identifier.__Unknown
...
internal class EnumAsApolloEnumBuilder(
  ...
  private fun IrEnum.toSealedInterfaceTypeSpec(): TypeSpec {
    return TypeSpec.interfaceBuilder(simpleName)
        ...
        .addSuperinterface(
            ClassName(packageName.experimental(), "ApolloEnum")
                .parameterizedBy(
                    selfClassName,
                    ClassName(packageName, simpleName, __Known)
                )
        )
  ...
  private fun IrEnum.Value.toObjectTypeSpec(): TypeSpec {
    return TypeSpec.objectBuilder(targetName.escapeKotlinReservedWordInSealedClass())
        ...
        .addSuperinterface(selfClassName.nestedClass(__Known))
        .addProperty(
            PropertySpec.builder("rawValue", KotlinSymbols.String)
                .addModifiers(KModifier.OVERRIDE)
                .initializer("%S", name)
  ...
  private fun knownValueTypeSpec(): TypeSpec {
    return TypeSpec.interfaceBuilder(__Known)
        .addKdoc("An enum value that is known at build time.")
        .addSuperinterface(selfClassName)
        .addSuperinterface(ClassName(packageName.experimental(), "KnownEnum").parameterizedBy(selfClassName))
        .addProperty(
            PropertySpec.builder(rawValue, KotlinSymbols.String)
                .addModifiers(KModifier.OVERRIDE)
                .build()
  ...
  private fun unknownValueTypeSpec(): TypeSpec {
    return TypeSpec.classBuilder(__Unknown)
        ...
        .addFunction(
            FunSpec.builder("toString")
                ...
                .addCode("return \"$__Unknown(${'$'}$rawValue)\"")
                .build()
        )
        .build()
  }
  ...
  private fun IrEnum.safeValueOfFunSpec(): FunSpec {
    return FunSpec.builder(safeValueOf)
        .addKdoc(
            ...
            The returned value may be an instance of [$__Unknown] if the enum value is not known at build time. 
