...
class FlattenKtTest {
  ...
  @Test
  fun testUpdateFromRebuildNoMatch() {
    // Fake data
    val testIrModelGroup = IrModelGroup(
      baseModelId = "extra",
      models = listOf()
    )

    // Prior to updateFromRebuild(), the connection should look like this:
    // tailIrModelGroup -> tailIrModel -> fakeStashedIrModelGroup
    val fakeStashedIrModelGroup = IrModelGroup(
      baseModelId = "IMG_a3",
      models = listOf(
        createTestIrModel("A", "B",) { listOf() }
      )
    )
    val tailIrModel = createTestIrModel("IM_i", "IM_i") {
      listOf(
        fakeStashedIrModelGroup,
        testIrModelGroup
      )
    }
    val tailIrModelGroup = IrModelGroup(
      baseModelId = "IMG_nn",
      models = listOf(
        tailIrModel
      )
    ...
    // Simulate a scenario where the previous operation did not flatten anything linked to tailIrModel
    val modifiedIrModelGroup = fakeStashedIrModelGroup.copy(
      baseModelId = "IMG_nope",
      models = emptyList()
    )
    val modifiedIrModelGroups = mutableListOf(modifiedIrModelGroup)

//    // Test update from updateFromRebuild() call
//    val testResults = updateFromPreviousRebuild(tailIrModelGroup, tailIrModel, modifiedIrModelGroups)
//
//    // After updateFromRebuild(), the connection should look like this:
//    // tailIrModelGroup -> tailIrModel -> fakeStashedIrModelGroup
//    Truth.assertThat(modifiedIrModelGroups).hasSize(1)
//    Truth.assertThat(testResults).isNull()
  }

  @Test
  fun testUpdateFromRebuildMatch() {
    // Fake data
    val testIrModelGroup = IrModelGroup(
      baseModelId = "extra",
      models = listOf()
    ...
    // Simulate a scenario where the previous operation flattened fakeStashedIrModelGroup
    val modifiedIrModelGroup = fakeStashedIrModelGroup.copy(
      models = emptyList()
    )
    val modifiedIrModelGroups = mutableListOf(modifiedIrModelGroup)
...
/*    val testResults = updateFromPreviousRebuild(tailIrModelGroup, tailIrModel, modifiedIrModelGroups)
    ...
    // After updateFromRebuild(), the connection should look like this:
    // tailIrModelGroup -> tailIrModel -> modifiedIrModelGroup
    Truth.assertThat(modifiedIrModelGroups).hasSize(0)
    Truth.assertThat(testResults!!.baseModelId).isEqualTo("IMG_nn")
    testResults.models.forEach {
      Truth.assertThat(it.id).isIn(listOf("extra", "IM_i"))
    }
    testResults.models.first { it.id == "IM_i" }.modelGroups.forEach {
      if (it.baseModelId == "IMG_a3") {
        Truth.assertThat(it.models).isEmpty()
      } else {
        Truth.assertThat(it.baseModelId).isEqualTo("extra")
      }
    }*/
  }
