...
This method handles fetch and GraphQL request errors but ignores GraphQL field errors. If a GraphQL field error happens, the corresponding Kotlin property is `null` and an error is present in `response.errors`. Handling those errors in your UI code can be complicated, and you can handle them earlier by looking at `response.errors`. 

## Handling GraphQL field errors

To handle GraphQL field errors, use `response.errors`:
...
state = when {
  ...
  response.exception is ApolloNetworkException -> {
    // Network error
    LaunchDetailsState.Error("Please check your network connectivity.") // highlight-line
  }
  response.data != null -> {
    // No error 🎉
    LaunchDetailsState.Success(response.data!!)
    ...
    LaunchDetailsState.Error("Oh no... An error happened.") // highlight-line
  }
}
```
