...
If a GraphQL field error happens, the corresponding Kotlin property is `null` and an error is present in `response.errors`: your response contains partial data! Handling this data in your UI code can be complicated. You can handle them earlier by looking at `response.errors`. 
