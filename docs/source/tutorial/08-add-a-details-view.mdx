...
For protocol errors, Apollo Android will throw a `ApolloException` so you'll need to wrap the call in a try/catch block:
...
`response.errors` will contain the details about the errors. Note that this code also checks for `response.data == null`. In theory, a server should not set `response.data == null` and `response.hasErrors == false` at the same time but the type system cannot guarantee this so better safe than sorry.
...
Put back the correct launch ID: `LaunchDetailsQuery(id = args.launchId)` before displaying details.
