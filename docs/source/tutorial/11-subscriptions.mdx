...
> By default, the tutorial uses the [`subscriptions-transport-ws`](https://github.com/apollographql/subscriptions-transport-ws/blob/master/PROTOCOL.md) protocol for subscriptions. For more options like the newer [`graphql-ws`](https://github.com/enisdenjo/graphql-ws/blob/master/PROTOCOL.md) or Appsync, see [GraphQL over WebSocket protocols](../essentials/subscriptions/#customizing-your-websocket-protocol).
