...
This tutorial uses `4.0.0-beta.6` because it is the latest version at the time of writing. Feel free to use a more recent version if one is available.

## Apply the plugin

Add the Apollo plugin to the version catalog in `gradle/libs.versions.toml`. The Apollo plugin ID is `com.apollographql.apollo3`:
