...
Sandbox Explorer tries to be helpful by adding the word `Mutation` after the word `Login`, but the Android SDK also does that for you - so you'd wind up with a `LoginMutationMutation`. Rename the operation `Login` instead: 
...
```kotlin:title=app/src/main/java/com/example/rocketreserver/LoginFragment.kt
            ...
            lifecycleScope.launchWhenResumed {
                val response = try {
                    apolloClient.mutate(LoginMutation(email = email))
                } catch (e: Exception) {
                    null
                }
