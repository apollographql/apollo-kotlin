...
And configure your `ApolloClient` with a cache-enabled `OkHttpClient` and `CacheUrlOverrideInterceptor`:

```kotlin
// Replace
val apolloClient = ApolloClient.Builder()
    .serverUrl(mockServer.url())
    .httpCache(directory = dir, maxSize = 10_000_000)
    .build()
...
val apolloClient = ApolloClient.Builder()
    ...
    .okHttpClient(
        OkHttpClient.Builder()
            .cache(Cache(directory = dir, maxSize = 10_000_000))
            .build()
    )
    .build()
```
...
There is no equivalent to `NetworkFirst` and/or `httpExpireTimeout()`. If you have a use case for them, please [open an issue](https://github.com/apollographql/apollo-kotlin/issues/new). 