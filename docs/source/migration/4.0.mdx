...
`customScalarsMapping` is removed and replaced with `mapScalar()` that allows mapping to built-in types easier and/or provide a compile type adapter for a given type:
