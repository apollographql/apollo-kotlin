...
**Version 4.0 is currently under development and this page itself is a work in progress.**
...
While most of the common APIs stayed the same, 4.0 isn't binary compatible with 3.x so we [changed the package name](https://github.com/apollographql/apollo-kotlin/issues/4710) to `com.apollographql.apollo`. 

Apollo Kotlin 4.0 removes some deprecated symbols. We strongly recommend removing deprecated usages before migrating to 4.0.  
...
Apollo Kotlin 4.0 ships with a companion [Android Studio plugin](../testing/android-studio-plugin#migration-helpers) that automates most of the migration. 
...
Apollo Kotlin 4.0 uses a new identifier for its package name, Gradle plugin id, and maven group id: **`com.apollographql.apollo`**.

This also allows to run 4.0 and 3.x side by side if needed.

In most cases, you can update the identifier throughout your project by performing a find-and-replace and replacing `com.apollographql.apollo3` with `com.apollographql.apollo`.

### Group id

The maven group id used to identify Apollo Kotlin 4.0 [artifacts](https://repo1.maven.org/maven2/com/apollographql/apollo/) is `com.apollographql.apollo`:
...
Apollo Kotlin 4.0 classes are imported from the `com.apollographql.apollo` package:
...
Error handling changes are a behavior change that is not detected at compile time. Usages of `execute`, `toFlow` and `watch` must be updated to the new error handling or changed to their 3.x compat equivalent.
...
To ease the migration from 3.x, drop-in helpers functions are provided that restore the 3.x behavior:
...
Because of the number of different options in 3.x and the complexity of error handling, these functions may not 100% match the 3.x behavior, especially in the advanced cases involving watchers. If you are in one of those cases, we strongly recommend using the 4.0 functions that are easier to reason about.
...
In 3.x, if HTTP headers were set on an `ApolloCall`, they would replace the ones set on `ApolloClient`. In 4.0 they are added instead by default. To replace them, call `ApolloCall.Builder.ignoreApolloClientHttpHeaders(true)`.
...
For the initial 4.0 release, most artifacts are kept with deprecation warnings and will be removed in the future.
...
In Apollo Kotlin 3.x, you could depend on an upstream GraphQL module by using the `apolloMetadata` configuration.

In Apollo Kotlin 4.0, this is now done with the `Service.dependsOn()`. This allows better management of dependencies when multiple services are used as better symmetry with `isADependencyOf` below. 
...
To avoid this, in 3.x you could manually specify which types to generate by using `alwaysGenerateTypesMatching`. In 4.0 this can now be computed automatically by detecting which types are used by the downstream modules.
...
Apollo Kotlin 3.x was using the operation root directories to compute the schema normalized path which could be wrong in some edge cases. Using fileTree ensures the normalized path is consistent.
...
In Apollo Kotlin 3.x, maven publishing was automatically configured. In Apollo Kotlin 4.0, this is no longer the case, and you need to configure the publishing tasks yourself.
...
In Apollo Kotlin 4.0, all directive usages are validated against their definition. If you have queries using client side directives:
...
In Apollo Kotlin 4.0, use `kotlinx-coroutines-rx${version}` instead. Replace the artifact in your build script:
