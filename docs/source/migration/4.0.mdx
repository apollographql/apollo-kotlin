...
With GraphQL it is possible to receive partial data (both `data` and `errors` are filled), but clients sometimes don't want to handle it and prefer to consider any errors as fatal. To help with this, an option `ApolloClient.ignorePartialData` has been added to force `ApolloResponse.data` to be `null` even if it wasn't null in the response, when there are errors. This can simplify error handling at the call site, e.g.:
