...
Apollo Android 3 provides a few helpers and compatibility modes to ease the migration from 2.x. To quickly reach a working state, follow the steps below.
Once you have a working app, we strongly recommend to migrate to idiomatic Apollo Android 3 as described in the [All details](#all-details) section below. The compatibility helpers will be removed in a future version of Apollo Android
...
Apollo Android 3.x exposes more concise coroutines APIs that handle cancellation automatically through the coroutine
scope.
...
As with 2.x, Apollo Android 3.x represents operation variables as optional parameters.
...
// 3.x
class GetHero(val id: Optional<String?> = Optional.Absent)
```
...
That is why Apollo Android 3.x provides a mechanism to remove the `Optional` wrapper, which makes the construction of queries with nullable variables more straightforward.
...
Apollo Android 2.x converts enums and enum values names to uppercase. 
Apollo Android 3.x uses the schema case for both enums and enum value names. This allows to define multiple enum values with a different case:
...
For performance reasons, Apollo Android 3.x uses the primitive type when possible and moves `BigDecimal` to the `apollo-adapters` artifact. This change should be transparent for most users because the generated parsers will convert Json numbers to `Int` or `Double` internally.
