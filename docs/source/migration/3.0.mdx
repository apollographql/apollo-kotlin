...
Apollo Android 3.x uses the schema case for both enums and enum value names. This allows to define multiple enum values with the difference case:
...
For performance reasons, Apollo Android 3.x uses the primitive type when possible and moves `BigDecimal` to the `apollo-adapters` artifact. This change should be transparent for most users because the generated parsers will convert `BigDecimal` to `Int` or `Double` internally.
