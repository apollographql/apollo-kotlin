...
Apollo Android 3.x uses `Optional` instead so that it can be used in places besides input types. For example, fields can be made optional with an [`@optional`](https://github.com/apollographql/apollo-android/blob/5c1ff82ccaded93ae7126cedc6d7835e63fed75a/apollo-compiler/src/main/resources/apollo.graphqls#L6) directive.
...
> **Note**: This new behavior applies _only_ to variables. Fields of input objects still use `Optional`, because it's common to omit particular input fields.
