...
## New package name, group id, and plugin id

Apollo Android 3.0 uses a new identifier for its package name, maven group id, and Gradle plugin id: **`com.apollographql.apollo3`**.
...
> You can probably update this identifier throughout your project by performing a find-and-replace on the 2.x identifier, `com.apollographql.apollo`.
...
> At the time of writing, Java codegen is not working but will be added later. For updates, see [this GitHub issue](https://github.com/apollographql/apollo-android/issues/2616).
...
### Removed compilation units

Apollo Android 2.x creates multiple `CompilationUnit`s for different Android variants or different Kotlin source sets. This runs the GraphQL compiler multiple times. For example, on Android, the library generates the models at least twice (once for the debug build and once for the release build). Most of the time, the generated GraphQL files are identical and this needlessly increases the build time.

Apollo Android 3.x removes `CompilationUnit`s. Instead, each `Service` is exactly one compilation. For Android projects, classes are generated once and then added to all variants.

If you need different operations for different variants, you can create multiple services for each Android variant like so:
...
Apollo Android 3.x instead uses Kotlin default arguments and exposes the constructors directly as top-level functions.
...
The Apollo Android 2.x runtime has a depency on the normalized cache APIs, and it's possible to call cache methods even if no cache implementation is in the classpath.
