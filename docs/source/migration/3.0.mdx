...
As a convenience and to ease the transition, you can also use the `ApolloClient.Builder.addCustomTypeAdapter` method to register a
`CustomTypeAdapter` that will convert between the old and the new APIs.
