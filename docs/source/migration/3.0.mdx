...
While the concepts stay the same, a lot of APIs have changed to work better in Kotlin.
...
You can fallback to the 2.x behaviour with:
...
For more control, you can also define your own `PackageNameGenerator`
...
You can create multiple services for each Android variants with:
...
Apollo Android 2.x runtime has builtin the normalized cache APIs and it is possible to call cache methods even if no cache implementation was in the classpath.
...
Similarly to the normalized cache, the HTTP cache is configurable through extension functions.
...
* `CacheKey.NONE` is replaced by `null`
