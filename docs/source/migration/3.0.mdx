...
// With
apolloClient.subscription(query).toFlow()
```
...
Apollo Android 2.x converts enum names to uppercase. This is no longer the case with Apollo Android 3.x, which uses the
same case for enum names as their GraphQL definition. This enables you to define different enums with different cases,
which is valid in GraphQL.
