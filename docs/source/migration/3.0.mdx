...
```
4. Queries and other operations:
```kotlin
// Replace:
...
For performance reasons, Apollo Android 3.x uses the primitive type when possible and moves `BigDecimal` to the `apollo-adapters` artifact. This change should be transparent for most users because the generated parsers will convert `BigDecimal` to `Int` or `Double`.

If you're reading the internal `Maps`, you can still be exposed to this change. This happens for an example, if you have a `CustomTypeAdapter` that reads nested values. In that case, you can do as below:
