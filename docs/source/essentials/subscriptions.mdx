...
* **SSE,** using the community [Avian Labs network transport](https://github.com/avianlabs/apollo-sse)
...
val apolloClient = ApolloClient.Builder()
    .subscriptionNetworkTransport(
        WebSocketNetworkTransport.Builder()
            .protocol(GraphQLWsProtocol()) // highlight-line
            .serverUrl("https://example.com/graphql")
            .build()
    )
    .build()
...
The coroutine scope that collects your subscription handles termination.

When the coroutine scope is canceled, the subscription is also terminated:
...
// when closing your app/screen, the job gets cancelled and the subscription is terminated.
job.cancel()
```

Most of the time, this is handled automatically by [lifecycle scopes](https://developer.android.com/topic/libraries/architecture/coroutines).
