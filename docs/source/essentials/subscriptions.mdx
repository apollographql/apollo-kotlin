...
Subscriptions are long-lived GraphQL read operations that can update their response over time, enabling clients to receive updates as they occur. Apollo Kotlin supports multiple transports for subscriptions:

* WebSockets using one of the following subprotocols:
...
## Configuring WebSockets subscriptions

To use subscriptions over WebSockets, use `WebSocketNetworkTransport` 
...
The GraphQL spec does not specify a particular protocol to use for subscription operations. By default, Apollo Kotlin uses [subscriptions-transport-ws](https://github.com/apollographql/subscriptions-transport-ws/blob/master/PROTOCOL.md) for backward compatibility purposes, but it supports all the following:
...
To use subscriptions over WebSockets, use `HttpNetworkTransport`
...
This is the only thing required. `HttpNetworkTransport` will use multipart for subscriptions and regular GET/POST for queries and mutations.
