...
> _Note_: Apollo Android supports both `https://` (or `http://`) and `wss://` (or `ws://`) protocols. Internally, `wss://` is renamed to `https://` and which one you use does not matter.
...
By default, Apollo Android uses [subscriptions-transport-ws](https://github.com/apollographql/subscriptions-transport-ws/blob/master/PROTOCOL.md) as a protocol. The subscription protocol is not specified by the GraphQL specification so you might come across different variants:
...
To customize your protocol, use the `WsProtocol` interface. Apollo Android comes with builtin support for the protocols above:
