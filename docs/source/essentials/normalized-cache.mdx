...
val resolver: CacheKeyResolver = object : CacheKeyResolver() {
  override fun fromFieldRecordSet(field: CompiledField, recordSet: Map<String, Any>): CacheKey {
    // Retrieve the id from the object itself
    return CacheKey(recordSet["id"] as String)
  }

  override fun fromFieldArguments(field: CompiledField, variables: Operation.Variables): CacheKey {
