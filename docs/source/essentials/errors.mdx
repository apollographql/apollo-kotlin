...
This is also true when calling `toFlow()` (e.g. with [subscriptions](/kotlin/essentials/subscription)) and `watch()` (with the [normalized cache](/kotlin/caching/query-watchers)).
