...
Network errors are surfaced in the `ApolloResponse.exception` field.
...
Another option is to call `ignorePartialData(true)`. This will force `ApolloResponse.data` to be null even if it wasn't in the response, whenever there are errors. This can also simplify error handling:

```kotlin
val data = apolloClient.query(query).ignorePartialData(true).execute().data
if (data == null) {
  // All network or GraphQL errors are handled here
} else {
  // No errors
...
```
This can also be set globally with `ApolloClient.Builder.ignorePartialData(true)`.
