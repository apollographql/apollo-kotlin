...
In addition, the `com.apollographql.apollo:apollo-adapters` artifact provides these adapters:
...
```kotlin title="build.gradle[.kts]"
dependencies {
  implementation("com.apollographql.apollo:apollo-adapters:4.0.0-beta.7")
}

apollo {
  service("service") {
    mapScalar("Date", "java.util.Date", "com.apollographql.apollo.adapter.DateAdapter")
  }
}
```
