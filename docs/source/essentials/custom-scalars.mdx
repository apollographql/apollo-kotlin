...
val geoPointAdapter = object : ScalarAdapter<GeoPoint> {
...
If you prefer working with `Map`s, Apollo Kotlin comes with `AnyScalarAdapter`, which supports adapting `String`, `Int`, `Double`, `Boolean`, `List`, and `Map`. You can use it in an intermediate step:
...
You can also register adapters on your `ApolloClient` instance by calling `ApolloClient.Builder.addScalarAdapter` once for each adapter:

```kotlin
val apolloClient = ApolloClient.Builder().serverUrl("https://")
    .addScalarAdapter(GeoPoint.type, geoPointAdapter)
    .build()
```
...
| Adapter                                            | Description                                       |
|----------------------------------------------------|---------------------------------------------------|
| `com.apollographql.apollo3.api.FloatScalarAdapter` | Converts from/to `kotlin.Float`/`java.lang.Float` |
| `com.apollographql.apollo3.api.LongScalarAdapter`  | Converts from/to `kotlin.Long`/`java.lang.Long`   |
...
For example, to use `DateScalarAdapter`, configure your Gradle scripts like so:

```kotlin title="build.gradle[.kts]"
dependencies {
  implementation("com.apollographql.apollo3:apollo-adapters:4.0.0-dev.2")
}

apollo {
  service("service") {
    mapScalar("Date", "java.util.Date", "com.apollographql.apollo3.adapter.DateScalarAdapter")
  }
}
```
