...
In addition to its [built-in scalar types](https://graphql.org/learn/schema/#scalar-types) (`Int`, `String`, etc.), GraphQL supports defining [custom scalars](/apollo-server/schema/custom-scalars/). For example, your schema might define a custom scalar for `Long`, `Date`, `BigDecimalAdapter`, or `GeoPoint`.
