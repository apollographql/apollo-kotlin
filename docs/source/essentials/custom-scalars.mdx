...
> If needed, you can also do this with built-in scalars (like `ID`) to override the type used by default.
...
```kotlin title="build.gradle[.kts]"
dependencies {
  implementation("com.apollographql.apollo3:apollo-adapters:3.2.0")
}
