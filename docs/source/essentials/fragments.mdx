---
title: Fragments in Apollo Android
sidebar_title: Fragments
---

> **Note:** This article describes the behavior of fragments when using the default `operationBased` codegen in Apollo Android. For the `responseBased` codegen, see [this page](../../advanced/response-based-codegen).

Apollo Android supports both forms of [GraphQL fragments](https://graphql.org/learn/queries/#fragments):
...
Based on the `fragment` definition, Apollo Android generates the following `LaunchFragment` class that you can reuse in different queries:
...
For this operation, Apollo Android generates a `Hero` class that contains `OnDroid` and `OnHuman` nested classes. `Hero` also includes `onDroid` and `onHuman` fields that enable you to access fields that are specific to `Droid` and `Human`:
