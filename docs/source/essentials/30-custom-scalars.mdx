---
title: Custom scalar types in Apollo Android
...
To interact with custom scalars in your Apollo Android app, you need to define a mapping in your `build.gradle` file. This tells Apollo Android which class to use to represent each custom scalar from your schema.
...
If you prefer working with `Maps`, Apollo Android comes with `AnyAdapter`, which supports adapting `String`, `Int`, `Double`, `Boolean`, `List`, and `Map`. You can use it in an intermediate step:
...
call `ApolloClient.Builder.customScalarAdapter` once for each adapter:

```kotlin
val apolloClient = ApolloClient.Builder().serverUrl("https://")
    .customScalarAdapter(Types.GeoPoint, geoPointAdapter)
    .build()
```
...
```kotlin
val apolloClient = ApolloClient.Builder().serverUrl("https://")
    .customScalarAdapter(Types.Date, DateAdapter())
    .build()
```
