---
title: Mutations in Apollo Android
sidebar_title: Mutations
---

import {MultiCodeBlock} from 'gatsby-theme-apollo-docs';

**Mutations** are GraphQL operations that _modify_ back-end data. As a convention in GraphQL, [queries](./00-queries/) are read operations and mutations are write operations.
...
val response = try {
  apolloClient.mutate(upvotePostMutation).execute()
...
In many cases, you'll want to use mutation results to update your UI. [Fragments](./50-named-fragments) are a great way to share result handling between queries and mutations:
...
The GraphQL type system includes [input objects](http://graphql.org/learn/schema/#input-types) as a way to pass complex values to fields. Input objects are often used for mutation variables, because they give you a compact way to pass in objects to be created:
...
try {
  val response = apolloClient.mutate(CreateReviewForEpisodeMutation(episode = Episode.NEWHOPE, review = reviewInput)).execute()
} catch(e: ApolloException) {
  // handle exception
}
```
