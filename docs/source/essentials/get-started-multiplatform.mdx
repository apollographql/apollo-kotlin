...
If you have any conflicts when resolving a dependency, this solution can help you - in your root `build.gradle.kts`, add the following:
```
allprojects {
    configurations {
        all {
...
So by bumping to 1.5.1, Gradle now sees 1.5.1-native-mt as a downgrade and fails. 
