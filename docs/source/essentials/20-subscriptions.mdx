...
Queries are useful to fetch data from a server. Mutations are useful to modify data. Subscriptions allow to listen to data changes coming from a server. They work by opening a channel with the server and receiving updates as they are pushed by the server, usually using a websocket.
