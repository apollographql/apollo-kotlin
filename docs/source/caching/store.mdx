...
Apollo Android exposes the `ApolloStore` API to read and write from the normalized cache programmatically. The `ApolloStore` sits on top of `NormalizedCache`, exposes a thread safe API as well as methods that make it easier to read and write fragments and operations.
