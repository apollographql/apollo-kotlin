---
title: Programmatic cache IDs in Apollo Android
sidebar_title: Programmatic cache IDs
---
...
Apollo Android provides a class for generating cache keys from each of these sources.
...
The `CacheKeyResolver` class enables you to generate custom cache IDs from an field's arguments. This basic example generates _every_ object type's cache ID from the `id` argument _if it's present_:
