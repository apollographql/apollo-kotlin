---
title: Programmatic cache keys in Apollo Kotlin
---

In situations where [declarative cache keys](./declarative-ids/) don't fit your use case, you can **programmatically** generate cache keys for object types in your normalized cache.

You can generate a given object type's cache key from one of two sources:
...
The `CacheKeyGenerator` class enables you to generate custom cache keys from an object's field values. This basic example generates _every_ object type's cache key from its `id` field:

```kotlin
val cacheKeyGenerator = object : CacheKeyGenerator {
  override fun cacheKeyForObject(obj: Map<String, Any?>, context: CacheKeyGeneratorContext): CacheKey? {
    // Generate the cache key based on the object's id field
...
You can get the current object's typename from the `context` object and include it in the generated key, like so:
...
You can also use the current object's typename to use _different_ cache key generation logic for different object types.

Note that for cache key generation to work, your GraphQL operations must return whatever fields your custom code relies on (such as `id` above). If a query does not return a required field, the cache key will be inconsistent, resulting in data duplication.
...
The `CacheKeyResolver` class enables you to generate custom cache keys from a field's arguments. This basic example generates _every_ object type's cache key from the `id` argument _if it's present_:
