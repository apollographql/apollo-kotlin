...
First, configure the cache to support optimistic updates (they are disabled by default, to avoid the overhead of keeping track of the optimistic data if you don't need it):

```kotlin {2}
val apolloClient = ApolloClient.Builder()
  .serverUrl("https://example.com/graphql")
  .cache(cacheFactory, enableOptimisticUpdates = true)
  .build()
```

Then, provide optimistic data with the `optimisticUpdates` method, like so:
