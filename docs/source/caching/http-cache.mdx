  ...
  val apolloClient = ApolloClient.Builder()
    .networkTransport(
        HttpNetworkTransport.Builder()
            .serverUrl(url)
            .httpEngine(
                OkHttpEngine(cachePostRequests = true) { // highlight-line
...
Under the hood, the OkHttp `HttpEngine` uses `"${url}?httpKey=${body.sha256()}"` as a cache key, allowing to cache POST requests.
