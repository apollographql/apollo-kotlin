...
dependencies {
  implementation("com.apollographql.apollo3:apollo-http-cache:4.0.0-beta.1")
}
```
...
Once your cache setup is complete, the cache will be used by default by all your queries (mutations are never cached). By default, queries will try to find a result in the cache first and go the network if it's not there. This is the `HttpFetchPolicy.CacheFirst`. You can customize that behaviour with `httpFetchPolicy(HttpFetchPolicy)`:
...
Call below to clear the cache of all entries, note that retrieving `apolloClient.apolloStore` will throw an exception if a cache has not been configured on the client.

```kotlin
try {
    apolloClient.apolloStore.clearAll()
} catch (e: IllegalStateException) {
    println("Failed to clear Apollo cache because none was found")
}
```