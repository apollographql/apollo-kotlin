...
The `@typePolicy` and `@fieldPolicy` directives are not part of your backend schema, so you need to import them in an `extra.graphqls` file in the same directory as your schema, before using them:
...
We can add the following definition to the `extra.graphqls` file:
...
```
"Author:Pierre+Bordage": {"id": "au456", "firstName": "Pierre", "lastName": "Bordage"}
```
