---
title: Declarative cache keys in Apollo Kotlin
---

When using a [normalized cache](./normalized-cache/) in Apollo Kotlin, it's recommended that you specify a **cache key** for each object type in your schema. If you don't, objects are assigned a _default_ cache key, but that key can lead to undesirable duplication of data.

When specifying cache keys, it's recommended that you do so **declaratively**, using the methods described in this article:
...
With declarative cache keys, the codegen process adds key fields automatically to your operations and generates type-safe code that can return a valid cache key for any object.
...
You can obtain a given object type's cache key from one of two sources:
...
The `@typePolicy` directive enables you to specify an object's cache key from the **key fields** of the object returned by your GraphQL server. Most commonly, you can use an object's `id` field as its key field.
...
Apollo Kotlin now knows to use the `id` field of a `Book` type to generate its cache key. A cache record may now look like
...
In this case, the cache key for an `Author` object includes the values of both its `firstName` and `lastName` fields:
...
The `@fieldPolicy` directive enables you to specify an object's cache key from the values of **key arguments** you provide to a particular field. This enables you to identify an object in your cache _before_ sending a network request, potentially enabling you to skip the request entirely.
...
In this case, the cache key for an `Author` object includes the values of both its `firstName` and `lastName` fields, which are both provided as arguments to the `Query.author` field.
