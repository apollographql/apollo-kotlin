...
After you add a normalized cache to your `ApolloClient` initialization, Apollo Kotlin automatically uses `FetchPolicy.CacheFirst` as default (client-wide) policy for all queries. In order to change it, you need to call **fetch policy** on the client builder:
...
You can also customize _how_ the cache is used for a particular query by setting a **fetch policy** for that query.
