---
title: Normalized caches in Apollo Android
sidebar_title: Normalized caches
---

Apollo Android provides two built-in **normalized caches** for storing and reusing the results of GraphQL operations:
...
```yaml:title=Cache
"favoriteBook": {"id": "bk123", "title": "Les guerriers du silence", "author": "CacheKey{favoriteBook.author}"
"favoriteBook.author": {"id": "au456", name": "Pierre Bordage"}
"QUERY_ROOT": {"favoriteBook": "CacheKey(favoriteBook)"}
```
...
* Notice that the `author` field of the `Book` entry now contains the string `CacheKey{favoriteBook.author}`. This is a **reference** to the `Author` cache entry.
...
Apollo Android's `MemoryCache` is a normalized, in-memory cache for storing objects from your GraphQL operations. To use it, first add the `apollo-normalized-cache` artifact to your dependencies in your `build.gradle[.kts]` file:
...
Apollo Android's SQLite cache uses [SQLDelight](https://github.com/cashapp/sqldelight) to store data persistently. You can use it to persist data across app restarts, or if your cached data becomes too large to fit in memory.
...
Then include the SQLite cache in in your `ApolloClient` initialization **according to your platform target** (different platforms use different drivers):
...
Whenever Apollo Android attempts to read cached data, it checks each chained cache in order until it encounters a hit. It then immediately returns that cached data without reading any additional caches.

Whenever Apollo Android _writes_ data to the cache, those writes propagate down _all caches_ in the chain.

## Setting a fetch policy

After you add a normalized cache to your `ApolloClient` initialization, Apollo Android automatically uses it by default for all queries. You can customize _how_ the cache is used for a particular query by setting a **fetch policy** for that query.

The following snippet shows how to set all available fetch policies and their behavior:
...
Because the normalized cache deduplicates data, it enables you to react to cache changes. You do thiswith `watchers` that listen to cache changes. [Learn more about query watchers.](./query-watchers/)

## Specifying cache IDs

By default, Apollo Android uses an object's GraphQL field path as its cache ID. For example, recall the following query and its resulting cache entries from earlier:
...
```yaml{2-3}:title=Cache
"favoriteBook": {"id": "bk123", "title": "Les guerriers du silence", "author": "CacheKey{favoriteBook.author}"
"favoriteBook.author": {"id": "au456", name": "Pierre Bordage"}
"author(\"id\": \"au456\")": {"id": "au456", "name": "Pierre Bordage"}
"QUERY_ROOT": {"favoriteBook": "CacheKey(favoriteBook)", "author(\"id\": \"au456\")": "CacheKey{author(\"id\": \"au456\")}"}
```
...
```yaml:title=Cache
"Book:bk123": {"id": "bk123", "title": "Les guerriers du silence", "author": "CacheKey{Author:au456}"
"Author:au456": {"id": "au456", name": "Pierre Bordage"}
"QUERY_ROOT": {"favoriteBook": "CacheKey(Book:bk123)", "author(\"id\": \"au456\")": "CacheKey{Author:au456}"}
```
...
Note that `id` can be obtained from two sources:

* From a response object's fields (e.g., `Book.id`)
    * This happens _after_ a network request and is essential to merging a query result with existing cached data. This is the most common case.
* From a GraphQL operation's arguments (e.g., `author(id: "au456")`)
    * This happens _before_ a network request and enables you to avoid a network round trip if all requested data is in the cache already. This is an optional optimization that can avoid some cache misses.

### Methods

There are two methods for specifying how to set an object's cache ID:
