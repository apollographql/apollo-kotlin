...
From there you can either drag and drop a `.db` file to the tool window to open it, or click on "Open file..." to pick the file to open.

There is also a "Pull from device" option to pull the cache database from a connected device or emulator. 
To pull in-memory caches, consult [the Apollo Debug Server section](apollo-debug-server).
