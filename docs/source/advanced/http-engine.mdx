...
And customizing the `HttpEngine`. For an example, you can use the builtin JVM client:
...
val apolloClient = ApolloClient.Builder()
    ...
    // If you use WebSocket subscriptions, you must also provide a custom `WebSocketEngine`. 
    .build()
```
