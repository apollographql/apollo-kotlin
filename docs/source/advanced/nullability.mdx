...
`@ignoreErrors` is an operation directive that disable error aware parsing for that operation. Effectively, it's as-if error aware parsing was disabled.
...
Enabling error aware parsing and adding `@ignoreErrors` to all your operation is a no-op operation. You can then progressively remove `@ignoreErrors` from your operations progressively. 
...
B --> V
A --> |NO| D[<a href='http://google.com'>Import the `@catch` directive</a>\n]
...
classDef question fill:#3F20BA,stroke:#0A061E,stroke-width:2px;
classDef action fill:#1B7471,stroke:#0A061E,stroke-width:1px;
classDef info fill:#135250,stroke:#0A061E,stroke-width:1px;
classDef success fill:#0F417A,stroke:#0A061E,stroke-width:2px;
```
