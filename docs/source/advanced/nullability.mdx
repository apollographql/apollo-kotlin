---
title: Handling nullability and partial data
...
* [`@semanticNonNull`](#semanticnonnull) (type-system directive)
* [`@catch`](#handle-errors-and-receive-partial-data-with-catch) (executable directive)
...
For the fields that are only nullable for error reasons. For those cases, Apollo Kotlin supports `@semanticNonNull`.
...
Define a field as semantically non null by adding the `@semanticNonNull` directive on its definition:
...
With `@semanticNonNull`, a frontend developer knows that a given field is never null unless there is an error and can therefore act accordingly. The Apollo Kotlin codegen generates `@semanticNonNull` fields as non-null Kotlin properties. No need to guess anymore! 
...
For these situations, extend your schema by using `@semanticNonNullField` in your extra.graphqls file:
