...
GraphQL does not have a `Result` type. If a field errors, it is set to `null` in the JSON response:
...
In the exceptional case that an error happens, the server returns the following response:
