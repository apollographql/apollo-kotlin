...
To help deal with those issues, Apollo Kotlin offers three tools:
* error aware parsing
* `@catch`
* `@nullOnlyOnError` 
...
Even with automatic detection of errors and handling of partial data like described above, you still have to check for  `null` in your UI code. This is because GraphQL by default recommends nullable fields. From the [GraphQL best practices](https://graphql.org/learn/best-practices/#nullability):
...
This makes sense because JSON cannot encode errors and it's easier to have a `data` field that matches the schema. 

But Kotlin can encode errors. For a field like 'User.name', it would make sense that it's always non-null unless something goes really wrong. 
