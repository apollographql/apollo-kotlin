...
When using authenticated subscriptions over WebSockets, you might want to refresh your token. Because it was invalidated or simply because it expired.

> Note: if your user logged out, you might want to consider creating a new `ApolloClient` instead to avoid sharing cache and/or other state between users.
...
```kotlin
    val apolloClient = ApolloClient.Builder()
        ...
        .webSocketReopenWhen { e, attempt ->
          if (WebSocketReconnectException) {
            true
          } else {
