...
Apollo Android supports batching multiple operations in a single HTTP request to reduce the number of network round trips. Whenever you execute an operation with batching enabled, your client waits a short time interval to collect any _other_ batchable operations that are also executed. When the interval completes, your client sends all of the operations in a single HTTP request.
...
By default, the batching engine waits 10ms to collect queries before sending them, and a single batch contains a maximum of 10 queries. You can configure these defaults when you initialize the `BatchingHttpEngine`, like so:
...
By default if you [enable batching](#enabling-batching), it's enabled for _all_ operations. An individual operation can opt _out_ of batching by passing `false` to the `canBeBatched` extension:
