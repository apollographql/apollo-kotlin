---
title: Persisted queries in Apollo Android
...
Apollo Android supports [Automatic Persisted Queries](https://www.apollographql.com/docs/apollo-server/performance/apq/) (APQ). To take advantage of this support, your GraphQL server _also_ needs to support APQ (Apollo Server supports it out of the box).
...
```kotlin
apolloClient.query(myQuery).hashedQuery(false).toFlow()
```
