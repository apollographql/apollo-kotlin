...
The operation manifest is generated in `build/gtask/generate${serviceName.capitalized()}ApolloSources/operationManifest.json`, where `$serviceName` is `"api"` here. The resulting operation manifest looks something like this:
