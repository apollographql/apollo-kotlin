...
## Generated methods
By default all Kotlin models, operations, fragments, and input objects are generated as data classes. This means that the compiler will
...
apollo {
  service("service") {
    ...
    modelMethods.set(listOf("equalsHashCode"))
    // Also generates toString, equals, and hashcode
    modelMethods.set(listOf("equalsHashCode", "toString"))
    // Only generates copy
    modelMethods.set(listOf("copy"))
    // Generates data classes
    modelMethods.set(listOf("dataClass"))
  }
}
```
