...
If you enabled operation safelisting on your backend (see [here] for more information about how to do this), you can use Apollo Android Gradle plugin to register your Android automatically. Apollo Android might transform the GraphQL files you write to include `__typename` (for polymorphic types) or trim whitespaces (to save some space). Registering your operation through the Gradle plugin makes sure the transformed version is registered so that there is an exact match between what is registered and what is sent by your app.
...
When your operations are stable and you want to safelist them, execute the `registerMain${serviceName}ApolloOperations` to push all your operation to the registry.
