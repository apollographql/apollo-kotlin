...
we built a GraphQL parser: a tool that can read, interpret, and expose GraphQL language constructs in a way that can be
used by the library.

Since this parser is not tied to the rest of the library, and as it may be useful for other purposes, we made it
available as its own artifact that you can use in your projects.
...
- AST can be translated back to valid GraphQL syntax
- Validation of schema and operations exposing warnings (e.g. use of deprecated fields) and errors
