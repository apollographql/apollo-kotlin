...
it parses your GraphQL files in an Abstract Syntax Tree ([AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree)). The
AST represents your GraphQL document in a type-safe, machine-readable way.
...
This method takes an implementation of the `NodeTransformer` interface defining the `transform` method. It will be
called with the visited `GQLNode` as a parameter and should return instructions to manipulate the AST:
