...
The `validateAsExecutable` method checks whether a document's defined operations are valid against a particular provided `Schema`. You can obtain this `Schema` parameter by calling teh [above](#validateAsSchema) `validateAsSchema` on the `GQLDocument` that represents the schema:
