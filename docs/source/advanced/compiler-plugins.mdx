...
In order to pass build-time arguments to your Apollo compiler plugin, use the `pluginsArguments` property:
...
apollo {
  service("service") {
    ...
    // Add argument
    pluginArguments.put("com.example.token", token) // highlight-line
  }
}
```
