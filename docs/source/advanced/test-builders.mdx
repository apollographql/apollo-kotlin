---
title: 'Using models in tests'
---

Apollo Android generates model classes based on your schema and operations. You can then use these classes in your
...
To make it easier, Apollo Android provides test builders: an optional mechanism that helps to build these models, with
default values.

> ⚠️ Note: test builders are experimental and subject to change.
...
This will generate a set of builder DSLs for each model used by your operations. These DSLs are based on the model's
...
and `height` are automatically filled with dummy values.
...
`DefaultTestResolver` will give String fields the fields' name as their value, increment a counter for Int values, etc.
