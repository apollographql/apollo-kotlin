...
Kotlin down to JS. `apollo-runtime` and supports JS interop out of the box with no code changes required.
...
To work around this, the Apollo compiler provides an option to skip the JSON parsing step completely on JS
platforms. To make use of this, you must set `jsExport.set(true)` on your service, and take careful note
