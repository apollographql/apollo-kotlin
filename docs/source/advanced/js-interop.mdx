...
* `jsExport` can be up to ~10x faster but does not support the Kotlin type system nor `operationBased` codegen.
* `DynamicJsJsonReader` can be up to ~2x faster but requires bypassing some parts of ApolloClient for JS.
