...
To work around this, Apollo Kotlin provides a `jsExport` option and a `DynamicJsJsonReader` class to work with
JS faster at the price of some limitations.
...
Because it bypasses some of the Kotlin type system, using `jsExport` comes with limitations.
See [Limitations](#Limitations) for more details.

### Usage

To use it, et `jsExport` to `true` in your Gradle scripts:
...
What's more, `@JsExport` have no runtime type information on JS so it's impossible to tell at runtime if a given
instance is a `Cat` ot a `Lion`. To check this, use `__typename` and `unsafeCastOrCast`:
...
* Uses `unsafeCast()` ([doc](https://kotlinlang.org/docs/js-interop.html#casts)) on JS. This does not type checking at
...
* `GraphQLList` are generated as `Array` instead of `List`.
