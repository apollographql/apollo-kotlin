...
By default, Apollo Kotlin will generate all the types in your schema module. This is because by default, there is no way to know in advance what types are going to be used by feature modules. 

For large schemas, this can generate a lot of code and increase your build time significantly. In this case, you can opt-in auto-detection of used types. This works by splitting the codegen task in different steps so that feature modules can let the schema know what types are used before actually generating the models. To opt-in, doubly link your projects using `isADependencyOf()`:
...
Once you opt-in auto-detection of used types, it's important that **all** modules are doubly linked. If not the feature modules will fail to compile due to some missing schema classes.