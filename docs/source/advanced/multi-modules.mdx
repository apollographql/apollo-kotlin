...
A feature module can have any number of apollo module dependencies, each one contributing their types and fragment.
...
If two sibling modules use the same schema type and this schema type wasn't generated upstream, each module will generate its own version of the schema type which could clash. To prevent this, Apollo Android registers a global "check${service}ApolloDuplicates" task that will fail if there are duplicates.
