...
The `com.apollographql.apollo3.network.websocket` implementation:

1. Defaults to the [graphql-ws](https://github.com/enisdenjo/graphql-ws/blob/master/PROTOCOL.md) protocol, which has become the de facto standard. Using the other protocols is still possible but having a main, specified, protocol ensures we can write a good test suite.
2. Does not inherit from the old memory model design, making the code considerably easier. In particular, `WebSocketEngine` is now event based and no flow control is done. If you buffers grow too much, your subscription fails.
3. Plays nicely with the ApolloClient `retryOnError` API.
4. Handles different Subscription transport more consistently.

## Status

While they are called "experimental", we believe the new APIS to be more robust than the non-experimental ones. They are safe to use in non-lib use cases.
