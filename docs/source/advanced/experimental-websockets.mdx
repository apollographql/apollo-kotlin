...
To account for non-websocket transports, like [multipart subscriptions](https://www.apollographql.com/docs/router/executing-operations/subscription-multipart-protocol/), the retry is now handled using [`ApolloClient.Builder.retryOnErrorInterceptor()`](https://apollographql.github.io/apollo-kotlin/kdoc/apollo-runtime/com.apollographql.apollo/-apollo-client/-builder/retry-on-error-interceptor.html) instead of `WebSocketNetworkTransport` previously.

Below is a more complete migration, customizing the connection payload and retry logic: 
        ...
        WebSocketNetworkTransport.Builder()
            ...
            .reopenWhen { throwable, attempt ->
              if (throwable is WebSocketReconnectException) {
                ...
                // Another WebSocket error happened, give up
                false
              }
            }
            .build()
...
Apollo Kotlin 4 also comes with a default `retryOnErrorInterceptor` that uses a network monitor or exponential backoff to retry the subscription. You can read more about it in the [network connectivity page](network-connectivity). 
