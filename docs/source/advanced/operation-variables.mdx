...
effect, optional can be opted back in on a per-variable basis, by setting the `@optional(if: true)` directive:

```graphql{1}
query GetTodos($first: Int @optional(if: true), $offset: Int @optional(if: true)) {
  todos(first: $first, offset: $offset) {
    id
    text
  }
