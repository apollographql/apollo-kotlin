...
You can opt out to optional variables globally in your Gradle config:
...
In that case, for the query seen above, Apollo Android generates the following Kotlin code:
...
This can also be configured on a per-variable basis, by setting the `@optional(if: false)` directive on the variables:
