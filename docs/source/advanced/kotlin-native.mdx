...
In particular, you must create your `ApolloClient` and call `query` and other operations from the main thread. Apollo Android then executes the work on a background thread and returns the result to the main thread.
