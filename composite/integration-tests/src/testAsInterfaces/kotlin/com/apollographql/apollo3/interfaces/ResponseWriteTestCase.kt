...
import com.apollographql.apollo3.integration.normalizer.EpisodeHeroWithInlineFragmentQuery.Data.Hero.Friend.Companion.asDroid
...
class ResponseWriteTestCase {
  ...
  fun operation_with_fragments() = runBlocking {
    ...
    val hero = HeroAndFriendsWithFragmentsQuery.Data.Hero(
        __typename = "Droid",
        id = "2001",
        name = "R222-D222",
        friends = listOf(
  ...
  fun operation_with_inline_fragments() = runBlocking {
    ...
    enqueueAndAssertResponse(
      ...
      assertThat(it.data?.hero?.name).isEqualTo("R2-D2")
      assertThat(it.data?.hero?.friends).hasSize(3)
      val asHuman = it.data?.hero?.friends?.get(0)?.asHuman()
      ...
      val asDroid1 = it.data?.hero?.friends?.get(1)?.asDroid()
      ...
      val asDroid2 = it.data?.hero?.friends?.get(2)?.asDroid()
    ...
    assertCachedQueryResponse(
      ...
      val asDroid = it.data?.hero?.friends?.get(1)?.asDroid()
