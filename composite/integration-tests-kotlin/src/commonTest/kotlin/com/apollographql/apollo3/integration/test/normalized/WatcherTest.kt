...
class WatcherTest {
  ...
  fun differentQueryTriggersWatcher() = runWithMainLoop {
    ...
    mockServer.enqueue(readResource("EpisodeHeroNameResponseWithId.json"))
    val job = launch {
      apolloClient.watch(EpisodeHeroNameWithIdQuery(Optional.Present(Episode.EMPIRE))).collect {
        channel.send(it.data)
      }
    }
  ...
  fun noChangeDifferentQuery() = runWithMainLoop {
    ...
    mockServer.enqueue(readResource("EpisodeHeroNameResponseWithId.json"))
    val job = launch {
      apolloClient.watch(EpisodeHeroNameQuery(Optional.Present(Episode.EMPIRE))).collect {
        channel.send(it.data)
      }
    }
  ...
  fun networkRefetchPolicy() = runWithMainLoop {
    ...
    mockServer.enqueue(readResource("EpisodeHeroNameResponseWithId.json"))
    val job = launch {
      val request = ApolloRequest(EpisodeHeroNameQuery(Optional.Present(Episode.EMPIRE)))
          .withFetchPolicy(FetchPolicy.NetworkOnly)
