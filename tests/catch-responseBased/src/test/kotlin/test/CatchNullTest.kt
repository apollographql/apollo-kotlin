...
import `null`.FragmentsQuery.Data.Companion.queryDetails
...
class CatchNullTest {
  ...
  fun nullError() {
    ...
    // nullable is `@catch(to: THROW)
    assertEquals(null, response.data)
    assertEquals("Oops", (response.exception as ApolloGraphQLException).error.message)
  }
  ...
  fun fragments() {
    ...
    assertEquals(42, response.data!!.nonNull?.plus(0))
    assertEquals(42, response.data!!.queryDetails()?.nonNull?.plus(0))
  }
  ...
  fun fragmentErrors() {
    ...
    assertEquals(null, response.data!!.nullable?.plus(0))
    assertEquals(null, response.data!!.queryDetails()?.nullable)
  }
  ...
  fun fragmentErrors2() {
    val response = FragmentsQuery().parseResponse("""
      {
        "errors": [{"message": "Oops", "path": ["nonNull"] }],
        "data": {"__typename": "Query", "nonNull": null, "nullable": null }
      }
    """.trimIndent())

    assertEquals(null, response.data!!.nonNull?.plus(0))
    assertEquals(null, response.data!!.queryDetails()?.nonNull)
  }
}
