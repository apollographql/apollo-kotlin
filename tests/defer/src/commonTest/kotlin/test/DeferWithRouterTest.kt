...
class DeferWithRouterTest {
  ...
  fun deferWithFragmentSpreadsMutation() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.mutation(WithFragmentSpreadsMutation()).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
  ...
  fun canDisableDeferUsingIfArgument() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.query(CanDisableDeferUsingIfArgumentQuery()).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
  ...
  fun doesNotDisableDeferWithNullIfArgument() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.query(DoesNotDisableDeferWithNullIfArgumentQuery(Optional.Absent)).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
  ...
  fun canDeferFragmentsOnTheTopLevelQueryField() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.query(CanDeferFragmentsOnTheTopLevelQueryFieldQuery()).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
  ...
  fun canDeferAFragmentThatIsAlsoNotDeferredDeferredFragmentIsFirst() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.query(CanDeferAFragmentThatIsAlsoNotDeferredDeferredFragmentIsFirstQuery()).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
  ...
  fun canDeferAFragmentThatIsAlsoNotDeferredNotDeferredFragmentIsFirst() = runTest(before = { setUp() }, after = { tearDown() }) {
    ...
    val actualDataList = apolloClient.query(CanDeferAFragmentThatIsAlsoNotDeferredNotDeferredFragmentIsFirstQuery()).toFlow().toList().map { it.data!! }
    assertEquals(expectedDataList, actualDataList)
  }
